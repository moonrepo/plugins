...
mod dotnet_tool {
    ...
    async fn resolves_aliases_from_the_index_alone() {
        ...
        //
        // `preview` is deliberately not in this list: whether a pre-GA channel
        // exists at all depends on where .NET is in its release cycle, so it
        // would make this test a time bomb. The phase handling behind it is
        // covered deterministically by the unit tests in `metadata`.
        for alias in ["lts", "sts", "latest"] {
            let output = plugin
