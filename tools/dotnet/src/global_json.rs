...
///
/// A feature band is the hundreds component of the patch, so it needs two
/// bounds rather than one (`>=8.0.404 && <8.0.500`). Across every mode
/// `rollForward` only ever rolls forward, so the pinned patch has to survive
/// into the spec. `latestPatch` is the SDK's default when `rollForward` is
/// absent, and it is band-locked.
///
...
/// which is close enough to the SDK's own behaviour.
