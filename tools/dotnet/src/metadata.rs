...
impl ChannelSummary {
    /// A channel that has not reached general availability.
    ///
    /// Two phases qualify. `preview` is the obvious one; `go-live` is the
    /// release-candidate phase, which carries a production-use licence but still
    /// ships an `-rc` SDK as its `latest-sdk`. Treating `go-live` as GA would
    /// hand an RC to anyone asking for `latest`, and would drop the `preview`
    /// alias the moment a channel moved from `preview` to `go-live`, which is
    /// exactly what .NET 11 did.
    pub fn is_prerelease(&self) -> bool {
        matches!(self.support_phase.as_str(), "preview" | "go-live")
    }

    /// A channel still receiving releases, which is what `lts`/`sts` should
    /// select. Excludes `eol` and anything pre-GA.
    ...
    pub fn latest_sdk_for_alias(&self, alias: &str) -> Option<&str> {
        ...
        let channel = match alias {
            ...
            "preview" => self
                .channels
                .iter()
                .find(|channel| channel.is_prerelease())?,
            // `latest` and `stable` mean the newest generally available SDK,
            // which is the highest channel that has shipped.
            "latest" | "stable" => self
                .channels
                .iter()
                .filter(|channel| !channel.is_prerelease())
                .max_by_key(|channel| channel.order_key())?,
            _ => return None,
        };
...
mod tests {
    ...
    /// Index shape taken from the live metadata the day .NET 11 moved out of
    /// `preview`, which is what exposed the `go-live` phase.
    const INDEX: &str = r#"{
      "releases-index": [
        {"channel-version": "11.0", "latest-sdk": "11.0.100-rc.1.26425.128",
         "support-phase": "go-live", "release-type": "sts"},
        {"channel-version": "10.0", "latest-sdk": "10.0.401",
         "support-phase": "active", "release-type": "lts"},
        {"channel-version": "9.0", "latest-sdk": "9.0.318",
         "support-phase": "maintenance", "release-type": "sts"},
        {"channel-version": "6.0", "latest-sdk": "6.0.428",
         "support-phase": "eol", "release-type": "lts"}
      ]
    }"#;

    fn index() -> ReleasesIndex {
        serde_json::from_str(INDEX).unwrap()
    }

    #[test]
    fn go_live_counts_as_prerelease() {
        // `go-live` is the release-candidate phase. Reading it as GA would make
        // `latest` resolve to an RC, and would lose the `preview` alias the
        // moment a channel left `preview` for it.
        let index = index();

        assert_eq!(
            index.latest_sdk_for_alias("preview"),
            Some("11.0.100-rc.1.26425.128")
        );
        assert_eq!(index.latest_sdk_for_alias("latest"), Some("10.0.401"));
        assert_eq!(index.latest_sdk_for_alias("stable"), Some("10.0.401"));
    }

    #[test]
    fn cadence_aliases_pick_the_highest_supported_channel() {
        let index = index();

        // 11.0 is sts but not GA, so `sts` stays on 9.0; 6.0 is eol, so `lts`
        // is 10.0.
        assert_eq!(index.latest_sdk_for_alias("lts"), Some("10.0.401"));
        assert_eq!(index.latest_sdk_for_alias("sts"), Some("9.0.318"));
        assert_eq!(index.latest_sdk_for_alias("nonsense"), None);
    }
