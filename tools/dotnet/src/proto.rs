...
pub fn load_versions(Json(_): Json<LoadVersionsInput>) -> FnResult<Json<LoadVersionsOutput>> {
    ...
    // One request per channel, EOL ones included. The `global.json` mapping
    // turns `rollForward` into a range, so a repository pinned to 3.1 or 6.0
    // needs those channels present here to resolve at all.
    //
    // proto caches what this returns, and `resolve_version` answers aliases
    // without reaching this function, so the full sweep is paid once per cache
    // expiry by range pins alone. The set cannot be narrowed by `input.initial`
    // either: proto caches one list per tool, so a spec-dependent subset would
    // be served to later, unrelated resolutions.
    //
...
pub fn resolve_version(
    ...
    // Returning a version here keeps `load_versions`, and its request per
    // channel, from running at all.
    //
    // Only aliases are answered. proto already short-circuits a fully qualified
    // version on its own, except for `proto install`, where it wants the
    // version validated against the real list before downloading anything.
    match &input.initial {
        // Every alias names a channel, and the index carries each channel's
        // `latest-sdk`, so one request answers it. An unknown alias falls
        ...
        // A requirement is matched against the real list, so it falls through.
        // Warn about a bare channel like `8.0`: feature bands are parallel
        // product lines, so it resolves to the highest band (4xx today) when
        // the repository may want 1xx. A band needs a compound requirement,
        // which is what the warning points at.
