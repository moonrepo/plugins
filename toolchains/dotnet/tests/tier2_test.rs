...
mod dotnet_toolchain_tier2 {
    ...
    mod extend_project_graph {
        ...
        async fn unsatisfiable_pin_degrades_when_moon_installs_the_sdk() {
            ...
            // moon merges its own `version:` into the toolchain config, and the
            // project graph is built before the action pipeline runs, so failing
            // here would deadlock the bootstrap this setting exists for.
            let mut input = projects_input();
            input.toolchain_config = json!({ "inferDependencies": true, "version": "8.0" });
            input.context = plugin.create_context();
