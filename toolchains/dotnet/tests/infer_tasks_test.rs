...
mod infer_tasks {
    ...
    #[test]
    fn granular_selection_is_respected() {
        let eval = evaluation(&[
            ("OutputType", "Exe"),
            ("BaseOutputPath", "bin\\"),
            ("TargetFramework", "net8.0"),
    ...
    #[test]
    fn resolves_unnormalized_msbuild_paths() {
        // MSBuild does not normalize the values it returns, so the common
        // shared-artifacts layout — `$(MSBuildThisFileDirectory)..\artifacts\`
        // in a `Directory.Build.props` — arrives with a literal `..`. moon
        // rejects any path containing `..`, and the resulting parse error used
        // to abort inference for the whole project.
        assert_eq!(
            resolve_output_path(
                "C:\\repo\\src\\..\\artifacts\\bin\\app\\",
                "C:\\repo\\src\\app",
                "C:\\repo"
            ),
            Some("/artifacts/bin/app".into())
        ...
        assert_eq!(
            resolve_output_path("/repo/src/../artifacts/bin", "/repo/src/app", "/repo"),
            Some("/artifacts/bin".into())
        );
        // Normalizing back into the project dir stays project-relative.
        assert_eq!(
            resolve_output_path("/repo/app/./bin/../bin/Debug", "/repo/app", "/repo"),
            Some("bin/Debug".into())
        );
        // A relative path leaving the project cannot be expressed to moon.
        assert_eq!(
            resolve_output_path("..\\artifacts\\bin", "C:\\repo\\app", "C:\\repo"),
            None
        );
        // Nor can one that climbs past the filesystem root.
        assert_eq!(
            resolve_output_path("/../elsewhere", "/repo/app", "/repo"),
            None
        );
        // Escaping the workspace entirely is still unresolvable.
        assert_eq!(
            resolve_output_path("C:\\repo\\..\\elsewhere\\bin", "C:\\repo\\app", "C:\\repo"),
            None
        );
    }
}
