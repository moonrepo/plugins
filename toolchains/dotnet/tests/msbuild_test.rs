...
mod suffix_matching {
    ...
    fn index(entries: &[(&str, Option<&str>)]) -> BTreeMap<String, Option<String>> {
        entries
            .iter()
            .map(|(suffix, id)| (suffix.to_string(), id.map(str::to_string)))
            .collect()
    }

    #[test]
    fn matches_on_the_tail_when_the_prefix_differs() {
        // The regression this exists for. The index is built from the paths the
        // host gave the plugin; the key comes from MSBuild. On a Windows runner
        // those disagree on the temp-dir prefix, and requiring the key to start
        // with the workspace root made every dependency edge vanish.
        let index = index(&[("/libs/common/common.csproj", Some("common"))]);

        assert_eq!(
            longest_suffix_match(
                &index,
                "c:/users/runneradmin/appdata/local/temp/ws/libs/common/common.csproj"
            ),
            Some(&"common".to_string())
        );
        assert_eq!(
            longest_suffix_match(&index, "d:/totally/elsewhere/libs/common/common.csproj"),
            Some(&"common".to_string())
        );
    }
    ...
    fn prefers_the_longest_suffix() {
        // A key can end with several indexed suffixes; the shorter belongs to a
        // different project, so first-in-map-order would be a wrong edge.
        let index = index(&[
            ("/lib/app.csproj", Some("lib")),
            ("/src/lib/app.csproj", Some("src-lib")),
        ]);

        assert_eq!(
            longest_suffix_match(&index, "c:/ws/src/lib/app.csproj"),
            Some(&"src-lib".to_string())
        );
    }

    #[test]
    fn skips_ambiguous_and_unmatched_entries() {
        let index = index(&[
            ("/a/shared.csproj", None),
            ("/b/other.csproj", Some("other")),
        ]);

        // Recorded ambiguous: two projects share the suffix, so neither wins.
        assert_eq!(longest_suffix_match(&index, "c:/ws/a/shared.csproj"), None);
        assert_eq!(longest_suffix_match(&index, "c:/ws/nope/none.csproj"), None);
    }
}
