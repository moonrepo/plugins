...
//!
//! The `dotnet` proto plugin parses this file too, and deliberately not through
//! this module. It needs an `UnresolvedVersionSpec` for proto to resolve
//! against; this needs the requirement itself — the raw string, the
//! `rollForward` mode and `allowPrerelease` — to judge whether an SDK already on
//! disk can serve the pin, and to detect which test runner the workspace
//! selected. One parser serving both would couple test-runner detection to
//! proto's version model for no gain.
