...
config_struct!(
    ...
    pub struct DotnetToolchainConfig {
        ...
        ///
        /// A task of the same id in a project's own `moon.yml` replaces the
        /// inferred one.
        ///
        /// An id coming from an inherited task file is different: moon merges
        /// the two and appends the args, which produces a command like
        /// `dotnet build -c Release build --no-restore`. Scope the task file
        /// away from .NET projects to avoid it, which is one edit for the whole
        /// workspace:
        ///
        /// ```yaml
        /// # .moon/tasks/<file>.yml
        ...
        /// ```
        ///
        /// Or handle it per project, where `rename` keeps both tasks and
        /// `exclude` discards the inherited one:
        ///
        /// ```yaml
        /// # moon.yml
        /// workspace:
