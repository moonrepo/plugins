...
pub fn resolve_output_path(raw: &str, project_dir: &str, workspace_dir: &str) -> Option<String> {
    ...
    // MSBuild hands back property values exactly as they were composed, without
    // normalizing them, so a `Directory.Build.props` setting
    // `$(MSBuildThisFileDirectory)..\artifacts\` yields a literal `..` segment.
    // moon rejects any path containing `..` outright, and because
    // `stable_inputs` propagates that parse error, one such property would abort
    // inference for the project and leave it with no tasks at all. Resolving the
    // segments here keeps that on the intended degradation path instead.
    let value = normalize_segments(value)?;
...
/// Resolve `.` and `..` segments lexically, keeping any leading `/` or drive
/// letter as a root that `..` cannot climb past.
///
/// `None` means the path escapes its own starting point, for a relative value
/// that is a directory outside the project, which no moon input or output can
/// express. Purely lexical on purpose: these paths need not exist yet (they are
/// build outputs), so there is nothing to canonicalize against.
fn normalize_segments(value: &str) -> Option<String> {
    let mut segments = value.split('/').peekable();

    let root = match segments.peek() {
        Some(first) if first.is_empty() || first.ends_with(':') => segments.next(),
        _ => None,
    };
    ...
    for segment in segments {
        match segment {
            // Empty segments come from doubled slashes.
            "" | "." => {}
            ".." => {
                parts.pop()?;
            }
...
/// MSBuild evaluation.
///
/// - `build` for every project, with `--no-dependencies` so moon's task graph
///   (`deps: ^:build`) orchestrates upstream builds and caches each project
///   independently. MSBuild resolves `ProjectReference`s from the upstream
///   `bin` output without rebuilding them.
/// - `test` for projects with `IsTestProject=true` or a
///   `Microsoft.NET.Test.Sdk` reference, with `--no-build` on a `build` dep.
/// - `run` for `Exe`/`WinExe` non-test projects, never cached, never in CI.
/// - `publish` for single-TFM `Exe`/`WinExe` non-test projects, since
///   multi-TFM `dotnet publish` requires an explicit `-f`.
///
/// `restore` is not a task. moon models it as the install-dependencies action,
/// which runs before tasks, hence `--no-restore` everywhere.
///
/// Ids in `reserved_ids` come from applicable inherited task files and are
/// skipped entirely, because moon merges a plugin task over an inherited one by
/// appending args, which produces a broken command. Project-level `moon.yml`
/// tasks need no such handling, as moon guarantees they win.
pub fn infer_tasks(
    setting: &InferTasksSetting,
