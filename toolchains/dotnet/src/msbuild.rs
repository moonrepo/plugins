...
/// The longest indexed suffix that `key` ends with, skipping entries recorded
/// as ambiguous (`None`).
///
/// Longest rather than first: a key can end with several indexed suffixes, and
/// the shorter one belongs to a different project, so map order would pick the
/// wrong one.
///
/// Only the tail is compared. The key comes from MSBuild and the index from the
/// paths the host gave the plugin, and the two need not agree lexically. On
/// Windows the same directory can appear in its long form on one side and its
/// 8.3 short form on the other, so anchoring the match to the workspace root
/// fails every lookup.
pub fn longest_suffix_match<'index, T>(
    index: &'index BTreeMap<String, Option<T>>,
    key: &str,
) -> Option<&'index T> {
    index
        .iter()
        .filter(|(suffix, value)| value.is_some() && key.ends_with(suffix.as_str()))
...
/// ```
///
/// The second form carries no line or column and is what SDK resolution
/// failures use, so missing it leaves a batch with no identifiable offender.
///
/// Only the trailing `<parent>/<file>` suffix is matched, not the full path,
/// because MSBuild prints expanded long paths that can differ lexically from
/// the ones passed in (Windows 8.3 short names such as `RUNNER~1` in a temp-dir
/// prefix). Both anchors keep the match at a token boundary so `App.csproj`
/// cannot match `MyApp.csproj`.
///
/// Lines are not filtered on `": error "`, since MSBuild localizes diagnostic
/// text and that would break on a non-English host. A path mentioned only in a
/// warning is therefore treated as failed, which over-excludes; an
/// over-excluded project falls back to per-project evaluation and stays
/// correct. The same applies to a suffix shared by two projects.
