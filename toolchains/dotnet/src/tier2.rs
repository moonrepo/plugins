...
pub fn locate_dependencies_root(
    ...
    // workspace, because one `dotnet restore` at the root covers the whole tree
    // beneath it. `None` would mean the root is the only package, leaving every
    // project below it outside the workspace.
...
pub fn parse_manifest(
    ...
    // A manifest with no parent directory has nothing to evaluate against.
    let Some(manifest_dir) = input.path.parent() else {
        return Ok(Json(output));
    };

    // `parse_manifest` carries no toolchain config, so `msbuildProperties`
    // cannot be honored here, the default config is passed deliberately. SDK
    // discovery is unaffected: an existing `DOTNET_ROOT`, proto's own installs
    // and the guarded `~/.dotnet` fallback all still apply.
