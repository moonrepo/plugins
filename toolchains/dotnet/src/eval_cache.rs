...
//! later, often in a separate process and against an already-cached project
//! graph, so it cannot rely on in-memory state. Without this, a workspace with no
...
/// is a routine Central Package Management migration, and it left the digest,
/// and therefore every task hash, unchanged.
///
...
/// switching or upgrading the system SDK reuses the old answer
