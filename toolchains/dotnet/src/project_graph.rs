...
struct ProjectIndexes {
    ...
    /// Normalized host-real workspace root, used to keep the suffix fallback
    /// from reaching outside the workspace. `None` when the root has no real
    /// path, which disables the fallback entirely rather than unanchoring it.
...
fn build_project_indexes(input: &ExtendProjectGraphInput) -> AnyResult<ProjectIndexes> {
    let mut indexes = ProjectIndexes {
        ...
        workspace_prefix: input
            .context
            .workspace_root
            .to_real_path()?
            .map(|root| normalize_path_key(&root.to_string_lossy())),
    };
...
/// index is built.
///
/// The fallback is anchored to the workspace root. The suffixes are only
/// workspace-*relative*, so an unanchored `ends_with` also matches paths outside
/// the workspace: a reference to `/elsewhere/libs/Common/Common.csproj` ends with
/// an in-workspace `libs/Common/Common.csproj` and would resolve to it. Since a
/// `None` from this function is exactly what marks a reference as living outside
/// the workspace, that mistake produced a wrong edge *and* silenced the
/// diagnostic that would have reported it.
fn resolve_reference<'index>(
    ...
    if let Some(id) = indexes.by_real_path.get(&key) {
        return Some(id);
    }
