...
pub fn extend_project_graph(
    ...
    // A resolved root counts even when nothing is on PATH: `build_eval_env`
    // points the invocation at that root's muxer, which is how a proto-managed
    // machine with no system install evaluates at all. Only when neither exists
    // is there nothing to evaluate with.
    if eval_env.dotnet_exe.is_none() && !command_exists(get_host_environment()?, "dotnet") {
        host_log!(
            warn,
            "No .NET SDK found, skipping .NET project graph evaluation — no dependency edges or inferred tasks will be contributed. Install a .NET 8+ SDK, or set <property>version</property> in <file>.moon/toolchains.yml</file> to have moon install one."
        );
