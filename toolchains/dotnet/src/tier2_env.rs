...
///
/// moon merges that setting into the `toolchain_config` every hook receives, so
/// it is read from there rather than from the file. It decides whether an
/// unresolvable SDK is a terminal misconfiguration or simply one that has not
/// been installed yet.
pub fn sdk_install_configured(toolchain_config: &serde_json::Value) -> bool {
    toolchain_config
        .get("version")
...
/// Resolve the DOTNET_ROOT that task environments and MSBuild evaluation both
/// use. They have to agree, or the graph is evaluated by one SDK while tasks
/// run under another.
///
/// Order: an existing host env var, then proto's installs, then `~/.dotnet`
/// when it holds a real SDK layout. Graph building needs the proto step because
/// it shells out to `dotnet` from the host PATH and is handed no tool
/// directory, so a proto-installed SDK is otherwise invisible to it.
///
/// Both discovered candidates are checked against the workspace `global.json`
/// pin, since an install that cannot serve the pin makes every task fail. An
/// explicit `DOTNET_ROOT` is taken as given.
///
/// Not memoized, even though it runs once per task. The project graph is built
/// before `SetupToolchain`, so a result cached during graph building predates
/// the SDK proto installs and would point later tasks at the wrong root.
...
///
/// A root counts only when it holds the `dotnet` host executable, plus an SDK
/// satisfying the workspace pin when `enforce_pin` says another root can take
/// its place.
fn usable_root(
    root: &VirtualPath,
    env: &HostEnvironment,
...
pub fn extend_task_command(
    ...
    // Only DOTNET_ROOT and PATH. Settings like `DOTNET_CLI_TELEMETRY_OPTOUT` and
    // `DOTNET_NOLOGO` belong in a task's own `env`, where they are visible.
