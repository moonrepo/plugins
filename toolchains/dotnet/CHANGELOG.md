...
- Initial release of the .NET toolchain plugin.
  ...
  - Tier 3: installation is delegated to the `dotnet` proto plugin, which this
    plugin embeds, so a configured `version` is installed and resolved the same
    way every other toolchain in this repository does it.
